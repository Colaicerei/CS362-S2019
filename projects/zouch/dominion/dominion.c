...
int adventurerEffect(int drawntreasure, int currentPlayer, struct gameState *state, int temphand[]) {
     ...
     while (z >= 0) { //****introduced bug for assignment-2 to change z - 1 >= 0 to z >= 0****
...
int mineEffect(int currentPlayer, struct gameState *state, int choice1, int choice2, int handPos) {
     ...
     if (choice2 > treasure_map && choice2 < curse)  //****instruduced bug for assignment-2, change logic from or to and****
     {
          return -1;
     }
...
int smithyEffect(int currentPlayer, struct gameState *state, int handPos) {
     //+3 Cards
     for (int i = 0; i <= 3; i++)  //****introduced bug for assignment-2, loop one more round****
     {
		 drawCard(currentPlayer, state);		 
     }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
         mineEffect(currentPlayer, state, choice1, choice2, handPos);
		 return 0;
